...
const server: GurgurServer | null = directory ? await createGurgurServer({
  port: 0,
  hostname: "127.0.0.1",
...
if (scenario === "grab" || scenario === "gamepad") await page.addInitScript(() => {
  const pad = {
...
if (scenario === "stale-session") await page.addInitScript(() => {
...
try {
  ...
  const requestedRevision = new URL(worldBundleRequests.at(-1) ?? url.href).searchParams.get("revision");
  if (!requestedRevision || !/^[0-9a-f]{64}$/.test(requestedRevision)) {
    throw new Error(`world bundle request is not revision-addressed: ${worldBundleRequests.at(-1) ?? "missing"}`);
  }
  ...
  if (scenario === "dynamic-landing") {
    await page.waitForFunction(
      (halfHeight) => {
        ...
        return Number.isFinite(supportY)
    ...
    await page.waitForFunction((y) => Number(document.body.dataset.predictedY) > y + 0.08, supportY);
    await page.waitForFunction(
      ({ halfHeight, jumpedFrom }) => {
        const target = Number(document.body.dataset.heavyCubeTopY) + halfHeight;
        return Number(document.body.dataset.predictedY) < jumpedFrom + 0.05
    ...
    await page.waitForFunction(() => Boolean(document.body.dataset.interactionTarget), null, { timeout: 5_000 });
    const beforeZ = Number(await page.evaluate(() => document.body.dataset.heavyCubeZ));
    await page.evaluate(() => { ((window as unknown as { __gurgurSmokePad: { buttons: Array<{ pressed: boolean; value: number }> } }).__gurgurSmokePad.buttons[7]!).pressed = true; });
    await page.waitForTimeout(40);
    await page.evaluate(() => { ((window as unknown as { __gurgurSmokePad: { buttons: Array<{ pressed: boolean; value: number }> } }).__gurgurSmokePad.buttons[7]!).pressed = false; });
    await page.evaluate(() => { ((window as unknown as { __gurgurSmokePad: { axes: number[] } }).__gurgurSmokePad.axes[1]!) = 1; });
    await page.waitForFunction((z) => Number(document.body.dataset.heavyCubeZ) > z + 0.12, beforeZ, { timeout: 4_000 });
    await page.evaluate(() => { ((window as unknown as { __gurgurSmokePad: { axes: number[] } }).__gurgurSmokePad.axes[1]!) = 0; });
    await page.evaluate(() => { ((window as unknown as { __gurgurSmokePad: { buttons: Array<{ pressed: boolean; value: number }> } }).__gurgurSmokePad.buttons[7]!).pressed = true; });
    await page.waitForTimeout(40);
    await page.evaluate(() => { ((window as unknown as { __gurgurSmokePad: { buttons: Array<{ pressed: boolean; value: number }> } }).__gurgurSmokePad.buttons[7]!).pressed = false; });
  } else if (scenario === "touch") {
    const groundedY = Number(await page.evaluate(() => document.body.dataset.predictedY));
    await page.dispatchEvent('[data-touch-action="jump"]', "pointerdown", { pointerType: "touch", pointerId: 41 });
    await page.dispatchEvent('[data-touch-action="jump"]', "pointerup", { pointerType: "touch", pointerId: 41 });
    await page.waitForFunction((y) => Number(document.body.dataset.predictedY) > y + 0.08, groundedY);
    const before = await page.evaluate(() => ({
      x: Number(document.body.dataset.predictedX), z: Number(document.body.dataset.predictedZ),
    }));
    await page.dispatchEvent("#world", "pointerdown", { pointerType: "touch", pointerId: 42, clientX: 120, clientY: 560 });
    await page.dispatchEvent("#world", "pointermove", { pointerType: "touch", pointerId: 42, clientX: 180, clientY: 500 });
    await page.waitForFunction(({ x, z }) => Math.hypot(
      Number(document.body.dataset.predictedX) - x, Number(document.body.dataset.predictedZ) - z,
    ) > 0.2, before);
    await page.dispatchEvent("#world", "pointerup", { pointerType: "touch", pointerId: 42, clientX: 180, clientY: 500 });
    ...
    await page.evaluate(() => { ((window as unknown as { __gurgurSmokePad: { axes: number[] } }).__gurgurSmokePad.axes[1]!) = 0; });
    const groundedY = Number(await page.evaluate(() => document.body.dataset.predictedY));
    await page.evaluate(() => { ((window as unknown as { __gurgurSmokePad: { buttons: Array<{ pressed: boolean }> } }).__gurgurSmokePad.buttons[0]!).pressed = true; });
    await page.waitForTimeout(40);
    await page.evaluate(() => { ((window as unknown as { __gurgurSmokePad: { buttons: Array<{ pressed: boolean }> } }).__gurgurSmokePad.buttons[0]!).pressed = false; });
    await page.waitForFunction((y) => Number(document.body.dataset.predictedY) > y + 0.08, groundedY);
  } else {
    const cadence = page.evaluate(() => new Promise<Array<{ x: number; z: number }>>((resolve) => {
      const samples: Array<{ x: number; z: number }> = [];
      const started = performance.now();
      const sample = (now: number): void => {
        samples.push({ x: Number(document.body.dataset.renderedX), z: Number(document.body.dataset.renderedZ) });
    ...
    await page.waitForFunction(
      ({ x, z }) => Math.hypot(Number(document.body.dataset.predictedX) - x, Number(document.body.dataset.predictedZ) - z) > 0.5,
      start,
    );
    const deltas = renderedSamples.slice(1).map((sample, index) => Math.hypot(
      sample.x - renderedSamples[index]!.x,
      sample.z - renderedSamples[index]!.z,
    )).filter(Number.isFinite);
    if (deltas.length >= 8 && deltas.filter((delta) => delta < 0.0005).length > deltas.length * 0.25) {
      throw new Error(`rendered prediction repeated too many display frames: ${JSON.stringify(deltas)}`);
    }
    if (simulatedLatencyMs >= 100) {
      ...
      if (Math.hypot(authorityDuringPrediction.x - start.x, authorityDuringPrediction.z - start.z) > 0.15) {
        throw new Error("authoritative movement arrived before the shaped-latency prediction check");
      }
    }
    const groundedY = Number(await page.evaluate(() => document.body.dataset.predictedY));
    await page.keyboard.press("Space");
  ...
  if (simulatedLatencyMs > 0) {
    ...
    const correction = Number(await page.evaluate(() => document.body.dataset.predictionCorrection));
  ...
  if (result.status !== "connected" || !result.canvasWidth || !result.canvasHeight || pageErrors.length > 0) {
  ...
  const latencyLabel = simulatedLatencyMs > 0 ? ` with ${simulatedLatencyMs * 2}ms simulated RTT` : "";
  console.log(`browser ${scenario} prediction smoke passed${latencyLabel} at tick ${result.tick} (${result.canvasWidth}x${result.canvasHeight})`);
} catch (error) {
  const failurePath = process.env.SMOKE_SCREENSHOT ?? join(tmpdir(), `gurgur-browser-${scenario}-failure.png`);
  await page.screenshot({ path: failurePath });
  console.error("browser smoke failure state", await page.evaluate(() => ({ ...document.body.dataset })));
