...
import { loadMaterialTextureManifest } from "../apps/server/src/material-textures";

await import("../tools/compile-map/src/index");

const materialTextureRoot = new URL("../content/textures/", import.meta.url);
const materialTextureManifest = await loadMaterialTextureManifest(materialTextureRoot);
...
for (const material of requiredMaterials) {
  if (!materialTextureManifest.textures[material]) {
...
console.log(
  `built ${result.outputs.length + workerResult.outputs.length} files, box3d.wasm, and ${materialTextureCount} authored textures`,
);
