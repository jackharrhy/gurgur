...
describe("map-authored authoritative world", () => {
  test("settles dynamic map hulls on compiled static collision and rebuilds them on reset", async () => {
    ...
    const game = await AuthoritativeGame.create(
      store,
      () => {},
      (world) => worldMessages.push(world),
    );
    try {
  ...
  test("drives a compiled trigger through its relay delay into a moving door", async () => {
    ...
    bootstrap.stop();
    const triggerEntity = bundle.entities.find(
      (entity) => entity.authoredId === "trigger.once.welcome",
    )!;
    ...
    const game = await AuthoritativeGame.create(
      ...
      () => {},
      () => {},
      { playerSpawn: triggerCenter },
    );
    try {
      game.connectPlayer();
      const door = game
        .worldMessage()
        .runtimeEntities.find((entity) => entity.authoredId === "mechanism.door.main")!;
      const initialY = game.snapshot().bodies.find((body) => key(body.id) === key(door.id))!
        .position.y;

      for (let tick = 0; tick < Math.ceil(0.75 / PHYSICS_DT); tick += 1) game.advance(PHYSICS_DT);

      const movedY = game.snapshot().bodies.find((body) => key(body.id) === key(door.id))!
        .position.y;
  ...
  test("persists trigger latches and queued relay work across a process restart", async () => {
    ...
    expect(saved.signals.find((signal) => signal.authoredId === "trigger.once.welcome")).toEqual({
      authoredId: "trigger.once.welcome",
      kind: "trigger",
      readyAtTick: 0,
      latched: true,
    });
    expect(saved.signals.find((signal) => signal.authoredId === "logic.relay.door")?.latched).toBe(
      true,
    );
    ...
    try {
      ...
      expect(store.load(restored.mapRevision)?.delayedSignals).toEqual([
        { target: "door.main", dueTick: 15 },
      ]);
      expect(
        store
          .load(restored.mapRevision)
          ?.signals.find((signal) => signal.authoredId === "trigger.once.welcome")?.latched,
      ).toBe(true);
  ...
  test("accepts a generation-safe, in-range, unobstructed button interaction", async () => {
    ...
    try {
      ...
      const button = game
        .worldMessage()
        .runtimeEntities.find((entity) => entity.authoredId === "mechanism.button.door")!;
      const door = game
        .worldMessage()
        .runtimeEntities.find((entity) => entity.authoredId === "mechanism.door.main")!;
      const initialY = game.snapshot().bodies.find((body) => key(body.id) === key(door.id))!
      ...
      const use = (sequence: number, generation: number) =>
        ({
          ...
          buttons: 0,
          jumpCounter: 0,
          interactCounter: sequence,
          interactTarget: { index: button.id.index, generation },
          primaryCounter: 0,
        }) as const;
      ...
      expect(
        game.snapshot().bodies.find((body) => key(body.id) === key(door.id))!.position.y,
  ...
  test("creates and releases a server-authoritative grab constraint on a visible dynamic body", async () => {
    ...
    try {
      ...
      const cube = game
        .worldMessage()
        .runtimeEntities.find((entity) => entity.authoredId === "physics.cube.heavy")!;
      const initialX = game.snapshot().bodies.find((body) => key(body.id) === key(cube.id))!
        .position.x;
      ...
      const pulledX = game.snapshot().bodies.find((body) => key(body.id) === key(cube.id))!
        .position.x;
  ...
  test("restores an owned grab constraint by authored body identity", async () => {
    ...
    try {
      ...
      const restoredCube = restored
        .worldMessage()
        .runtimeEntities.find((entity) => entity.authoredId === "physics.cube.heavy")!;
      const before = restored
        .snapshot()
        .bodies.find((body) => key(body.id) === key(restoredCube.id))!.position.x;
      for (let sequence = 0; sequence < 60; sequence += 1) {
        restored.acceptInput(restoredPlayer, {
          type: "input",
          protocolVersion: PROTOCOL_VERSION,
          worldEpoch: restored.worldEpoch,
          sequence,
          clientTick: sequence,
          moveX: 0,
          moveZ: -1,
          ...
          jumpCounter: 0,
          interactCounter: 0,
          interactTarget: restoredCube.id,
          primaryCounter: 0,
        });
        restored.advance(PHYSICS_DT);
      }
      expect(
        restored.snapshot().bodies.find((body) => key(body.id) === key(restoredCube.id))!.position
          .x,
  ...
  test("restores map bodies by authored identity", async () => {
    ...
    try {
      ...
      expect(loaded.bodies.map((body) => body.position)).toEqual(
        saved.bodies.map((body) => body.position),
      );
    } finally {
      restored.stop();
      store.close();
  ...
  test("restores an in-flight mechanism instead of returning to its authored start", async () => {
    ...
    const doorId = first
      .worldMessage()
      .runtimeEntities.find((entity) => entity.authoredId === "mechanism.door.main")!.id;
    ...
    try {
      const restoredDoor = restored
        .worldMessage()
        .runtimeEntities.find((entity) => entity.authoredId === "mechanism.door.main")!.id;
      const restoredY = restored
        .snapshot()
        .bodies.find((body) => key(body.id) === key(restoredDoor))!.position.y;
      expect(restoredY).toBeCloseTo(savedY, 4);
      restored.advance(PHYSICS_DT);
      expect(
        restored.snapshot().bodies.find((body) => key(body.id) === key(restoredDoor))!.position.y,
      ).toBeGreaterThan(restoredY);
    } finally {
      restored.stop();
      store.close();
    }
  ...
  test("owns generation-safe players and advances accepted input", async () => {
    ...
    try {
      ...
      expect(
        game.worldMessage().runtimeEntities.some((entity) => entity.classname === "player"),
      ).toBe(true);
  ...
  test("coalesces a burst to current held intent without losing monotonic action edges", async () => {
    ...
    try {
      ...
      const player = game
        .snapshot()
        .players.find((candidate) => key(candidate.id) === key(playerId))!;
  ...
  test("emits one final sleeping body delta and then keeps it silent", async () => {
    ...
    try {
      ...
      const sleep = appearances.find(
        (appearance) => (appearance.flags & SNAPSHOT_FLAG_SLEEP) !== 0,
      );
      expect(sleep).toBeDefined();
      expect(
        emitted
          .filter((snapshot) => snapshot.serverTick > sleep!.tick + 3)
          .every((snapshot) => snapshot.bodies.every((body) => key(body.id) !== key(cube.id))),
      ).toBe(true);
    } finally {
      game.stop();
      store.close();
  ...
  test("replicates nearby prediction bodies at 20 Hz and staggers unrelated dirty bodies at 10 Hz", async () => {
    ...
    try {
      ...
      expect(
        emitted.every((snapshot) =>
          snapshot.bodies.some((body) => key(body.id) === key(nearby.id)),
        ),
      ).toBe(true);
      expect(
        emitted.filter((snapshot) =>
          snapshot.bodies.some((body) => key(body.id) === key(unrelated.id)),
        ),
      ).toHaveLength(1);
    } finally {
      game.stop();
  ...
  test("builds a complete discontinuity snapshot for backpressure recovery", async () => {
    ...
    try {
      ...
      expect(
        snapshot.bodies.every((body) => ((body.flags ?? 0) & SNAPSHOT_FLAG_TELEPORT) !== 0),
      ).toBe(true);
    } finally {
      game.stop();
      store.close();
