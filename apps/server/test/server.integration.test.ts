...
describe("authoritative server", () => {
  test("serves health and streams advancing binary snapshots", async () => {
    ...
    try {
      ...
      const physicsDebugResponse = await fetch(
        `http://127.0.0.1:${server.port}/debug/physics?test=1`,
      );
      ...
      const authoredRuntimeCount = bundle.entities.filter((entity) => entity.body !== null).length;
      expect(world.runtimeEntities).toHaveLength(authoredRuntimeCount + 1);
      expect(world.runtimeEntities.some((entity) => entity.kind === "player")).toBe(true);
  ...
  test("gives sixteen concurrent clients unique authoritative players", async () => {
    ...
    try {
      ...
      expect(
        connections.at(-1)!.world.runtimeEntities.filter((entity) => entity.kind === "player")
          .length,
      ).toBe(16);
    ...
    try {
      ...
      expect(
        world.runtimeEntities
          .filter((entity: RuntimeEntityRef) => entity.kind === "world-entity")
          .every(
            (entity: RuntimeEntityRef) => !oldIds.has(`${entity.id.index}:${entity.id.generation}`),
          ),
      ).toBe(true);
