...
if (process.env.PUBLIC_ORIGIN) {
  ...
  if (!["http:", "https:"].includes(origin.protocol) || origin.origin !== process.env.PUBLIC_ORIGIN.replace(/\/$/, "")) {
...
const server = await createGurgurServer({
  port, adminToken, publicOrigin: process.env.PUBLIC_ORIGIN, extraDynamicBodies,
  playerSpawn: playerSpawn ? { x: playerSpawn[0]!, y: playerSpawn[1]!, z: playerSpawn[2]! } : undefined,
});
