...
export class PhysicsWorld {
  ...
  private constructor(box3d: Box3DModule) {
    this.#box3d = box3d;
    const definition = box3d.b3DefaultWorldDef();
    definition.gravity = { x: 0, y: -10, z: 0 };
  ...
  static async create(options?: { locateFile(path: string): string }): Promise<PhysicsWorld> {
    return new PhysicsWorld(await createBox3D(options));
  }
  ...
  createPlayerProxy(position: Vec3, options: { crouched?: boolean } = {}): RuntimeId {
    this.#assertLive();
    const definition = this.#box3d.b3DefaultBodyDef();
    ...
    const halfSegment = options.crouched ? 0.25 : PLAYER_CAPSULE_HALF_SEGMENT;
  ...
  moveCapsule(start: Vec3, desired: Vec3, options: { halfSegment?: number } = {}): Vec3 {
    this.#assertLive();
    const halfSegment = options.halfSegment ?? PLAYER_CAPSULE_HALF_SEGMENT;
    const capsule = {
      center1: { x: 0, y: -halfSegment, z: 0 },
      center2: { x: 0, y: halfSegment, z: 0 },
      radius: PLAYER_CAPSULE_RADIUS,
  ...
  castCapsule(start: Vec3, desired: Vec3, options: { halfSegment?: number } = {}): Vec3 {
    this.#assertLive();
    const halfSegment = options.halfSegment ?? PLAYER_CAPSULE_HALF_SEGMENT;
    const filter = this.#box3d.b3DefaultQueryFilter();
    filter.maskBits &= ~PLAYER_PROXY_CATEGORY;
  ...
  capsuleFits(center: Vec3, options: { halfSegment?: number } = {}): boolean {
    this.#assertLive();
    const halfSegment = options.halfSegment ?? PLAYER_CAPSULE_HALF_SEGMENT;
    const filter = this.#box3d.b3DefaultQueryFilter();
    filter.maskBits &= ~PLAYER_PROXY_CATEGORY;
