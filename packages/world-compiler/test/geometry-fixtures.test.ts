...
describe("Valve geometry conformance fixtures", () => {
  test.each([
  ...
  ] as Array<[string, Transform]>)("compiles deterministic finite %s brushes", (_name, transform) => {
    ...
    expect(first.brushes[0]?.worldVertices.every((vertex) => Object.values(vertex).every(Number.isFinite))).toBe(true);
  });
