...
describe("Valve 220 parser diagnostics", () => {
  ...
  test("reports file, line, column, entity, brush, and face", () => {
    expect(() => parseValve220(`{
"classname" "worldspawn"
"mapversion" "220"
{
  nope
}
}`, "broken.map")).toThrow(/broken\.map:5:3: face 0/);
  });
  ...
  test("fails deterministically under a seeded malformed-token corpus", () => {
    ...
    for (let caseIndex = 0; caseIndex < 512; caseIndex += 1) {
      ...
      let first: string;
      let second: string;
      try { first = JSON.stringify(parseValve220(source, `fuzz-${caseIndex}.map`)); } catch (error) {
        expect(error).toBeInstanceOf(Error);
        first = String(error);
      }
      try { second = JSON.stringify(parseValve220(source, `fuzz-${caseIndex}.map`)); } catch (error) {
        expect(error).toBeInstanceOf(Error);
        second = String(error);
      }
