...
const LIFECYCLE_ENTITY_BYTES = 13;
...
export function encodeLifecycle(message: LifecycleMessage): ArrayBuffer {
  ...
  const bytes = new ArrayBuffer(
    LIFECYCLE_HEADER_BYTES +
      message.created.length * LIFECYCLE_ENTITY_BYTES +
  ...
  for (const entity of message.created) {
    view.setUint8(offset, entity.kind === "world-entity" ? 1 : 2);
    view.setUint32(offset + 1, entity.id.index, true);
    view.setUint32(offset + 5, entity.id.generation, true);
    view.setUint32(
      offset + 9,
      entity.kind === "world-entity" ? entity.entityIndex : PLAYER_ENTITY_INDEX,
      true,
    );
    offset += LIFECYCLE_ENTITY_BYTES;
  }
...
export function decodeLifecycle(bytes: ArrayBuffer | ArrayBufferView): LifecycleMessage {
  ...
  const created: RuntimeEntityRef[] = [];
  let offset = LIFECYCLE_HEADER_BYTES;
  for (let index = 0; index < createdCount; index += 1) {
    if (offset + LIFECYCLE_ENTITY_BYTES > view.byteLength)
      throw new Error("lifecycle entity is truncated");
    const sourceTag = view.getUint8(offset);
    if (sourceTag !== 1 && sourceTag !== 2) throw new Error("lifecycle source tag is invalid");
    const id = {
      index: view.getUint32(offset + 1, true),
      generation: view.getUint32(offset + 5, true),
    };
    const entityIndex = view.getUint32(offset + 9, true);
    offset += LIFECYCLE_ENTITY_BYTES;
    if (sourceTag === 2) {
      if (entityIndex !== PLAYER_ENTITY_INDEX)
        throw new Error("lifecycle player entity index is invalid");
      created.push({ id, kind: "player" });
    } else {
      if (entityIndex === PLAYER_ENTITY_INDEX)
        throw new Error("lifecycle world entity index is invalid");
      created.push({ id, kind: "world-entity", entityIndex });
    }
  }
  const removed = [];
