...
const runs: Array<{
  ...
  ...[2, 8, 16, 32].map((clientCount) => ({
    ...
    options: {
      clientCount,
      durationMs: quick ? 700 : clientCount === 16 ? 5_000 : 1_500,
      seed: seed + clientCount,
      scenarioName: `gate-${clientCount}`,
    },
  })),
  {
  ...
  },
  {
    ...
    options: {
      clientCount: 16,
      durationMs: quick ? 1_200 : 3_000,
      seed: seed + 200,
      scenarioName: "receiver-stall",
      receiverPause: { clientIds: [1], untilMs: quick ? 500 : 1_500 },
    },
  },
  {
    ...
    options: {
      ...
      durationMs: quick ? 1_200 : 3_000,
      seed: seed + 300,
      scenarioName: "connected-reset",
      resetAtMs: quick ? 400 : 1_000,
...
for (const run of runs) {
  ...
  reports.push(report);
  await Bun.write(
    `reports/network/${run.name}-${seed}.json`,
    `${JSON.stringify(report, null, 2)}\n`,
  );
  console.log(JSON.stringify(summarizeHarnessReport(report)));
}

const correctnessErrors = reports.reduce(
  (sum, report) => sum + report.aggregate.correctnessErrors,
  0,
);
...
for (const report of reports.filter((candidate) => candidate.scenario.name === "gate-16")) {
  for (const [profile, budget] of Object.entries(budgets)) {
    ...
    if (!metrics) continue;
    if (metrics.predictionErrorP95Metres > budget.p95 + epsilon)
      budgetFailures.push(`${report.scenario.name}/${profile} prediction p95`);
    if (metrics.predictionErrorP99Metres > budget.p99 + epsilon)
      budgetFailures.push(`${report.scenario.name}/${profile} prediction p99`);
    if (metrics.predictionErrorMaxMetres > budget.max + epsilon)
      budgetFailures.push(`${report.scenario.name}/${profile} prediction max`);
    if (metrics.snapshotAgeP95Ms > budget.age)
...
for (const report of reports.filter((candidate) =>
  ["five-second-outage", "receiver-stall"].includes(candidate.scenario.name),
)) {
  if (report.scenario.recoveryPredictionSamples < 10)
...
if (correctnessErrors || tickBudgetFailures.length || budgetFailures.length) {
  const summary = [
    `${correctnessErrors} correctness errors`,
    `${tickBudgetFailures.length} tick-budget failures`,
    `budgets=${budgetFailures.join(",")}`,
  ].join(", ");
  throw new Error(`network matrix failed: ${summary}`);
}
